    ...
    return (
      ...
      <div className=" mt-4 md:mt-8">
        <div className=" text-center">
          <h1 className=" text-2xl md:text-5xl font-bold">{category}</h1>
          <p className="mt-0 md:mt-2 text-sm md:text-lg">{categoryDescription} </p>
          <button className=" border border-gray-200 mt-2 md:mt-4 rounded-2xl px-3 py-1">Total {filteredRecipes.length} Recipes</button>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 mt-8 md:mt-16 gap-4 gap-y-6 md:gap-y-8">
          {filteredRecipes.map((item) => (
            ...
            <FaHeart className="h-6 w-6 text-red-500" />
              ...
              {/* Details */}
              <div>
                <h2 className="mt-2 md:mt-4 font-semibold text-red-500">{item.foodName}</h2>
